...
def get_features(patchX, patchY, patchZ, weight):
    ...
    angle_p = math.atan2(v[1, 0], v[0, 0])
    angle_t = math.acos(v[2, 0] / (sqrt((v[0, 0]) ** 2 + v[1, 0] ** 2 + v[2, 0] ** 2) + 1e-16))
    ...
    return angle_p, angle_t, math.log(trace), fa
    ...
    for i1 in range(C.PATCH_HALF, H - C.PATCH_HALF):
        ...
        for j1 in range(C.PATCH_HALF, W - C.PATCH_HALF):
            for k1 in range(C.PATCH_HALF, D - C.PATCH_HALF):
                ...
                point_space[patchNumber] = np.array(get_features(patchX, patchY, patchZ, w))
                patchNumber += 1
