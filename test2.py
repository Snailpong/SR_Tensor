...
from train2 import load_kmeans_model, get_features
...
def make_image(im_LR, im_GX, im_GY, im_GZ, w, kmeans, h):
    ...
    for i1 in range(C.PATCH_HALF, H - C.PATCH_HALF):
        ...
        timer = time.time()
        fS = get_feature_yz(i1, result_image, im_LR, im_GX, im_GY, im_GZ, w)

        if len(fS) == 0:
            continue

        jS = kmeans.predict(fS)
        result_image = make_hr_yz(i1, result_image, im_LR, jS, h)
...
def get_feature_yz(i1, result_image, im_LR, im_GX, im_GY, im_GZ, w):
    ...
    for j1 in range(C.PATCH_HALF, W - C.PATCH_HALF):
        for k1 in range(C.PATCH_HALF, D - C.PATCH_HALF):
            ...
            fS.append(get_features(patchX, patchY, patchZ, w))

    return fS


@njit
def make_hr_yz(i1, result_image, im_LR, jS, h):
    H, W, D = im_LR.shape
    cnt = 0
